...
app.whenReady().then(async () => {
  ...
  /* ---- the samples are part of the set, and must match it ---- */
  const smp = await w.webContents.executeJavaScript(`(async () => {
    const MP = window.MixProject, DSP = window.MixDSP;
    if (!window.__project || !window.__buffersForTest) return { noHooks: true };
    const sr = 48000;

    /* three samples at wildly different levels, the way a set of clips cut
       on different nights actually arrives */
    const ctx = new OfflineAudioContext(2, sr, sr);
    function clip(amp, secs, hz) {
      const b = ctx.createBuffer(2, Math.floor(sr * secs), sr);
      for (let c = 0; c < 2; c++) {
        const d = b.getChannelData(c);
        for (let i = 0; i < d.length; i++) {
          const t2 = i / sr;
          d[i] = amp * (Math.sin(2 * Math.PI * hz * t2) + 0.4 * Math.sin(2 * Math.PI * hz * 2.5 * t2));
        }
      }
      return b;
    }
    const clips = [
      { id: 'tst_loud', name: 'loud one', buf: clip(0.70, 2.0, 300) },
      { id: 'tst_mid',  name: 'middling', buf: clip(0.14, 1.5, 420) },
      { id: 'tst_quiet',name: 'quiet one', buf: clip(0.03, 0.8, 250) }
    ];

    const p = window.__project();
    /* the library the UI reads from, and the audio cache behind it */
    if (window.__sampleCacheForTest) {
      const cache = window.__sampleCacheForTest();
      clips.forEach(c => cache.buffers.set(c.id, c.buf));
      clips.forEach(c => cache.meta.set(c.id, { id: c.id, name: c.name }));
    } else { return { noCache: true }; }

    p.placements.length = 0;
    clips.forEach(c => p.placements.push({
      sampleId: c.id, atJunction: 0, barsBeforeEntry: 4,
      gainDb: -8, fadeInMs: 40, fadeOutMs: 300, mode: 'over'
    }));

    document.getElementById('normaliseBtn').click();
    await new Promise(r => setTimeout(r, 1500));

    const landed = [];   /* what loudness does each one now land at */
    for (const c of clips) {
      const pl = p.placements.find(x => x.sampleId === c.id);
      const mono = DSP.toMono(c.buf);
      let l = DSP.loudness(mono, sr, 0, mono.length / sr);
      if (l == null) {
        let a = 0; for (let i = 0; i < mono.length; i++) a += mono[i] * mono[i];
        l = 20 * Math.log10(Math.sqrt(a / mono.length) + 1e-12) - 0.7;
      }
      landed.push({ name: c.name, own: +l.toFixed(1), gain: pl.gainDb, at: +(l + pl.gainDb).toFixed(1) });
    }
    return { landed: landed };
  })()`);

  console.log('\n  placed samples, levelled by the same button:');
  if (smp.noHooks || smp.noCache) {
    ok(false, 'the sample cache can be reached from a test', JSON.stringify(smp));
  } else {
    smp.landed.forEach(s => console.log('    ' + s.name.padEnd(12) +
      'arrived at ' + String(s.own).padStart(7) + ' LUFS, given ' +
      String(s.gain).padStart(6) + ' dB, so it lands on ' + s.at));
    const at = smp.landed.map(s => s.at);
    const spread = Math.max.apply(null, at) - Math.min.apply(null, at);
    const own = smp.landed.map(s => s.own);
    ok(Math.max.apply(null, own) - Math.min.apply(null, own) > 10,
       'the clips really did arrive at very different levels',
       (Math.max.apply(null, own) - Math.min.apply(null, own)).toFixed(1) + ' dB apart');
    ok(spread < 1.0,
       'and after levelling they all hit the same volume, like the records do',
       at.join(' / ') + ' LUFS, spread ' + spread.toFixed(1) + ' dB');
    ok(at.every(v => Math.abs(v - (-20)) < 1.0),
       'and it is the same number the records were levelled to',
       at.join(' / ') + ' against -20');
  }
